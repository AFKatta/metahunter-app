...
/**
 * The versions of one deck, and what changed between them.
 *
 * MTGO overwrites a deck file when you edit it, so without this the
 * list you registered last week ceases to exist and the games you
 * played with it lose their attribution. Each version is kept, with its
 * own record — a 3-2 on Tuesday and a 2-3 on Wednesday are two results,
 * not one 5-5, and merging them hides the only thing you wanted to
...
function when(ts: number): string {
  ...
  // Calendar days, not elapsed hours. Something played at 10pm last
  // night is fourteen hours old, which an elapsed-time check calls
  // "today" — and a version list that labels yesterday's league as
  // today is worse than showing a bare date.
...
export function VersionPicker({
  ...
  // A deck that has only ever had one list needs no picker; showing one
    ...
    <section className="rounded-xl border bg-card p-3">
      ...
      {pinned && (
        <p className="mb-2 text-[10px] leading-relaxed text-muted-foreground">
          Showing this list only. The record, matchups and league runs
          below are its own.
        </p>
      )}
              ...
              >
                <span className="min-w-0 flex-1 truncate">
                  ...
                  {v.source === "registration" && (
                    <span
                      className="ml-2 rounded border px-1 text-[9px] uppercase text-muted-foreground"
                      title="This list had already been overwritten in MTGO. It was recovered from a match you played with it."
                    >
                      recovered
                    </span>
                  )}
...
function summarise(v: DeckVersion): string {
  ...
  if (added === 0 && removed === 0) return "no card changes"
